...
export default function Canvas({
  ...
  const renderElement = (element: CanvasElement) => {
    ...
    if (element.type === 'world') return null;
  ...
  const worldElement = project.elements.find(el => el.type === 'world');
  const hasWorldImage = worldElement?.worldImage;

  const handleLocationClick = (location: any) => {
    if (!location.link || location.link.type === 'none') return;

    if (location.link.type === 'url') {
      if (location.link.openInNewTab) {
        window.open(location.link.target, '_blank');
      } else {
        window.location.href = location.link.target;
      }
    }
  };
  ...
  return (
    <div className="flex-1 overflow-auto p-8 bg-slate-800/50">
      ...
      >
        ...
        {worldElement?.locations?.map((location) => (
          <div
            key={location.id}
          ...
          >
            <button
              className="whitespace-nowrap select-none cursor-pointer hover:scale-105 transition-transform shadow-lg"
              style={{
